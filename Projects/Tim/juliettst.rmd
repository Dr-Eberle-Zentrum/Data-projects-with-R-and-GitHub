...
```{r Temperaturentwicklung in Tübingen (1961-2024) mit Trend }

klimadaten %>%
  filter(saison == "kalenderjahr")%>%
ggplot(aes(x = jahr, y = temperatur_mittelwert)) +  
...
```{r Saisonale Temperaturentwicklung (1961-2024)}
klimadaten %>%
  filter(saison ==c("winter_djf", "fruehjahr_mam", "sommer_jja", "herbst_son")) %>%
ggplot( aes(x = jahr, y = temperatur_mittelwert, color = saison)) +  
...
```{r Zusammenhang: Niederschlag und Heiße Tage}
...
Die Korrelation zwischen Niederschlag und heißen Tagen beträgt `` `r round(cor_niederschlag_heiss, 3)` `` .

### Wie entwickeln sich die Heißtage und Frost im laufe der Zeit und Sasional?

```{r Entwicklung Extrem-Temperaturtage (1961-2024)}
...
klimadaten %>%
         ...
         pivot_longer(cols = c(temperatur_heisse_tage, temperatur_frosttage),
                      names_to = "typ", 
                      values_to = "anzahl") %>%
ggplot(aes(x = jahr, y = anzahl, color = typ)) +
  geom_line(linewidth = 1) +
...
```{r Saisonale Trenddiagramme, alle Saisons in einem Diagramm}
...
ggplot(aes(x = saison, y = wert, fill = variable)) +
  geom_col() +
...
```{r Häufigkeit und Dauer von Trockenperioden}
...
klimadaten %>%
         ...
         mutate(periode_kategorie = cut(trockenperiode_tage_maximum,
                                       ...
                                       labels = c("0-10", "11-20", "21-30", 
                                                 "31-40", ">40"),
                                       right = TRUE)) %>%
ggplot(aes(x = jahr, y = periode_kategorie, fill = trockenperiode_tage_maximum)) +
...
klimadaten %>%
         filter(saison == "kalenderjahr") %>%
ggplot(aes(x = temperatur_tropennaechte, y = trockenperiode_tage_maximum)) +
  geom_bin2d(bins = 15) +
  scale_fill_gradient(low = "#fee5d9", high = "#a50f15", 
                      name = "Anzahl\nJahre") +
  labs(
...
klimadaten %>%
         filter(saison %in% "kalenderjahr") %>%
  ggplot(aes(x = niederschlag_summe, y = temperatur_mittelwert)) +
...
```{r Threshold Exceedance Plots}

klimadaten %>%
  filter(saison %in% tolower(month.name)) %>%
  mutate(
...
klimadaten %>%
  filter(saison %in% tolower(month.name)) %>%
  mutate(
    typ = "Monatliche Verteilung",
    saison_label = c("Jan", "Feb", "Mär", "Apr", "Mai", "Jun",
                     "Jul", "Aug", "Sep", "Okt", "Nov", "Dez")[match(saison, tolower(month.name))],
    x_wert = match(saison, tolower(month.name))
  ) %>%
  group_by(typ, saison, saison_label, x_wert) %>%
  summarise(wert = mean(starkniederschlag_20mm_tage, na.rm = TRUE), .groups = "drop") %>%
ggplot(aes(x = x_wert, y = wert, fill = wert)) +
  geom_col() +
