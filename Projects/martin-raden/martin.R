...
data <-
  ...
  # clean up
  mutate(
    # (!) cut of last character for integer conversion
...
data %>%
  ggplot(aes(y= Common, yend= Common)) +
  theme_minimal()+
  # disable horizontal grid lines
  theme(panel.grid.major.y = element_blank(),
        panel.grid.minor.y = element_blank()) +
  # (!) using constant 1 as start x coordinate
  geom_segment(aes(x=1, xend = 0.93*Average),
               col="gray",
               arrow = arrow(ends = "last", length = unit(5,"pt"), type = "closed"),
               arrow.fill = "gray") +
  geom_point(aes(x=1), col="ivory4") +
  ...
  scale_x_log10(breaks = 10^(0:4),
                ...
                labels = scales::percent_format(scale = 1, big.mark=","),
                limits=c(1,10^5)) +
  labs(
    x = "Average relative change in size of population (logarithmic scale)",
    y = NULL # (!) drop y axis label
  ) +
  # disable clipping to allow for annotations outside of plot area
  coord_cartesian(clip = "off")+
  # year range annotations
  geom_label(aes(label = Range, x=1.1),
             hjust=0, size=3, fill="white", label.size=0, col="darkgray" ) +
  # average change annotations after end points
  geom_text(aes(label= scales::percent_format(scale = 1, big.mark=",")(Average),
                x=Average*1.1),
            hjust=0, size=3)+
