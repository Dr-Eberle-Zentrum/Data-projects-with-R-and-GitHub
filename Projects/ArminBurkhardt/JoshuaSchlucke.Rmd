...
```{r read-and-clean, warning=FALSE}
...
- The hour with the highest average intrahour movement (US/Eastern) is:
  `r hourly_volatility %>% slice_max(avg_abs_move, n = 1, with_ties = FALSE) %>% pull(hour)`
  am (avg abs move:
  `r scales::percent(hourly_volatility %>% slice_max(avg_abs_move, n = 1, with_ties = FALSE) %>% pull(avg_abs_move), accuracy = 0.01)`).

- The strongest positive correlation with S&P 500 hourly returns is:
  `r correlation_df %>% slice_max(correlation, n = 1, with_ties = FALSE) %>% pull(Symbol)`
  (correlation:
  `r round(correlation_df %>% slice_max(correlation, n = 1, with_ties = FALSE) %>% pull(correlation), 3)`).

- The weakest correlation with S&P 500 hourly returns is:
  `r correlation_df %>% slice_min(correlation, n = 1, with_ties = FALSE) %>% pull(Symbol)`
  (correlation:
  `r round(correlation_df %>% slice_min(correlation, n = 1, with_ties = FALSE) %>% pull(correlation), 3)`).
