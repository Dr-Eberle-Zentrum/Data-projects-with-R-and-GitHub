...
Splitting the unit variable by NR and P_HTHAB.
Creating a new data set with the observed values from NR and P_HTHAB in the same row,
so we can use for y the P_HTHAB observed values and for the size the NR observed value.
(before I created this new data set, I tried using a ifelse function for "y" and the "size" in the aes function, but it wasn't working.
So, there is prob a more elegant way to do it :)
