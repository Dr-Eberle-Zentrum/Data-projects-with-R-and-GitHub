...
Here is an extract of the `Inventory` table:
    ...
    ## # A tibble: 6 × 40
    ##     Obj Field       POS      Context Number Phase HollowGlass FlatGlass Splitter
    ##   <dbl> <chr>       <chr>      <dbl> <chr>  <chr>       <dbl>     <dbl>    <dbl>
    ## 1    44 J           17-19 u…     249 xx1    <NA>           NA        NA       NA
    ## 2   851 Pl.Fl 43-46 557          466 201    Pink…           1        NA       NA
    ## 3  1030 14          335          297 xx1    Ober…          NA        NA       NA
    ## 4  1030 8           655          359 xx1    Pink…          NA        NA       NA
    ## 5  1030 13          365          399 201    Pink…           1        NA       NA
    ## 6  1030 10          633          441 201    Pink…           1        NA       NA
    ## # ℹ 31 more variables: GlassChunk <dbl>, Droplet <dbl>, Slag <dbl>,
    ## #   Tessera <dbl>, Rod_yellow <dbl>, Rod_orange <dbl>, Rod_red <dbl>,
    ## #   Rod_blue <dbl>, Rod_white <dbl>, Rod_greeen <dbl>, Rod_Molten <chr>,
    ## #   Rod_Plychrome <chr>, Rod_Length <dbl>, Rod_Dm <dbl>, Pontil_yellow <dbl>,
    ## #   Pontil_orange <dbl>, Pontil_red <dbl>, Pontil_bluee <dbl>,
    ## #   Pontil_white <dbl>, Pontil_green <dbl>, OGW_yellow <dbl>, OGW_orange <dbl>,
    ## #   OGW_red <dbl>, OGW_black <dbl>, OGW_blue <dbl>, OGW_green <dbl>, …
...
-   The first 4 columns contain the serial number of each object
    ...
    -   `Number` is the actual number of the object. The numbers are
        ...
        identifier of each object can be obtained my combining
...
-   All the other column represent the different object types. If
    ...
    corresponding column. In some case, the colours of the glass objects
    are also indicated.
...
Here is an extract of the table:
    ...
    ## # A tibble: 6 × 15
    ##     Obj Field   POS Context Number Phase    TypeNumber Type  Colour Form  Length
    ##   <dbl> <dbl> <dbl>   <dbl>  <dbl> <chr>         <dbl> <chr> <chr>  <chr>  <dbl>
    ## 1  1052    12   458     312    201 Pit hou…          3 doub… green  doub…      6
    ## 2  1052    12   458     312    202 Pit hou…         15 wind… red    asym…      5
    ## 3  1052    18  1460     858    201 Upper s…          3 doub… blue   doub…      8
    ## 4  1052    20  1072    1000    202 Pink - …          4 ring… green  pres…      8
    ## 5  1052    19  1724    1056    201 Pink - …         10 poly… yello… ring…      5
    ## 6  1052    19  1731    1111    201 Pink - …          1 cyli… red    cyli…      7
    ## # ℹ 4 more variables: Dm <dbl>, HoleSize <dbl>, IronOxide <chr>, Broken <chr>
...
-   `Length`, `Dm`, and `HoleSize` give the metric characteristics of
    each bead: length, diameter, and size of the tread hole. Some beads
    have missing parameters because they could not be measured (brocken
    beads) or simply were not measured yet.
...
1.  Add a column `SampleID` which gives an unique identifiers for each
    object. Place this column after `Number`.
...
3.  Transpose the table so that the object type and colour information
    are given in 2 new columns: `Type` and `Colour`.  
    To be honest, I am not yet quite sure how to do that in a
    non-painful way.

4.  The colour information for the beads can be taken from the
    `Beads`table (after `SampleID` is added to that table) using a
    `join()` function.  
...
changed to remove `Rod_` in them since this is not needed anymore.
...
`Beads` and `Rods`, we can not plot a new histogram of `Volume` per
`Colour` for both.  
