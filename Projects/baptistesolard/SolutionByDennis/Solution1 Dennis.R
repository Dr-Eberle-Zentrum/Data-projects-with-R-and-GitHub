...
import('Inventory_Glassworkshop.csv',
         setclass = 'tibble') -> inventory
import('GlassBeads.csv',
       setclass = 'tibble') -> GlassBeads
...
#2. Create a new dataframe "Rods" with selected columns from the original dataframe "inventory"
...
# Extract color information into new dataframe
...
#4. another full join to include all beads
...
# Define a custom color palette based on unique values in the "Colour" column
custom_palette <- unique(merged_BI$Colour)

# Filter the dataframe for each object type and remove NA values in the "Colour" column
