...
- `df_treat` = A data frame containing **multiple cells that received a treatment** and their **time points observations**. *intensity* and *area* are the same as in the individual cell data frames. *x* and *y* has been removed.

- `df_untreat` = A data frame containing **multiple cells from the control group** (cells that DON'T received treatment) and their time points observations. *intensity* and *area* are the same as in the individual cell data frames. *x* and *y* has been removed.
...
Here are plot one in top of the other. Note that the data is scaled so the plots have the same proportion. 


```{r, echo=FALSE}
dying_scale <- scale(dying_corr)
colnames(dying_scale)
dy_time <- c(dying_scale[,1])
dy_area <- c(dying_scale[,2])
dy_lumen <- c(dying_scale[,3])
...
We do find some positive correlation between *area and intensity* (**0.29**) but with not as significant.
...
abline(h=800, v=10000, col="orange")+
dim(dying_corr)
plot
}
...
time_x <- dying_scaled[1:1605,1]
dying_intensity <- dying_scaled[1:1605,2]
dying_area <- dying_scaled[1:1605,3]
...
Following Julia's recommendation and accordingly to what is shown in the plots above, **size (area) is a good indicator of a cell's point of death**. 
