...
final_data <- variance_data %>%
  ...
  mutate(
    color_factor_x = factor(round(center_x)),
    color_factor_y = factor(round(center_y))
  )
```
...
ggplot(aes(x = center_x, y = center_y)) +
  ...
  geom_point(aes(size = bubble_size, color = color_factor_x, fill = color_factor_y), shape = 21) +
  ...
  # color them
  scale_fill_manual(values = c("green", "greenyellow", "yellow", "orange", "darkred"))+
  scale_color_manual(values = c("green", "greenyellow", "yellow", "orange", "darkred")) +
  # change design
  theme_minimal() +
  theme(
