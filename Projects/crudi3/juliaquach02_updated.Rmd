...
# anstatt die Spalten explizit zu listen (für das select würde ich das Entwickeln einer entsprechenden regex vorschlagen, das alle spaltennamen, die du haben willst, beschreibt. die regex kannst du dann im select() mit matches() direkt verwenden. # damit dein spaltenrename via colnames() dann noch geht, musst du ggf. die ingle spalte via relocate() nach vorn holen..
...
#  deine result Berechnung lässt sich auf 3 Zeilen eindampfen, wenn du "im tidyverse Stil" rangehst. Im moment ist deine df Tabelle im wide table format und damit nicht "tidy", weil in jeder Zeile mehrere trust level Zahlen vorliegen. was du brauchst, um mit tidyverse darauf effizient rumzurödeln ist eine Tabelle, die 3 spalten hat: Inglehart-Index, Institution, Trust-Level. Die kannst du ganz einfach erzeugen, indem du die wide table tabelle via pivot_longer() ins long-table Format überführst...
#  anschliessend kannst du mit grouping und summarize() direkt die Mittelwerte pro Institution bestimmen. sprich df %>% privot_longer(..) %>% group_by(..) %>% summarize(...) -> result ! versuchs mal! 😀
...
# gleiches hilft dir nämlich dann auch bei deinem letzten coding: das grouping kann auch auf zwei spalten basieren, sprich Inglehart-Index UND Institution.. damit bekommst du dann direkt die Mittelwerte pro Kombination raus, etc...
...
#       scale_y_continuous( expand = c(0, 0), labels = 0:2, breaks= c(0, 1, 2) ) +
#       theme( plot.margin = margin(1,1,1,1.2, "cm") ) +
#       annotate("text", x = 0.5, y = 1.42, hjust = 0, size = 3, label = "max.Diff.") +
#       annotate("text", x = 8.5, y = 1.1, hjust = 0, size = 3, label = "min.Diff.") 
