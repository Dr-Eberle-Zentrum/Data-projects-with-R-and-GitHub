...
The Fellowship Of The Ring: `r words_spoken$total_words[words_spoken$Film == "The Fellowship Of The Ring"]`
The Return Of The King: `r words_spoken$total_words[words_spoken$Film == "The Return Of The King"]`
The Two Towers: `r words_spoken$total_words[words_spoken$Film == "The Two Towers"]`
...
Please visualize the distribution of the total number of words across the three volumes, using a donut chart
...
Please visualize this distribution using **three (3) pie charts**, similar to the one above. 

- Only show the **top 9 characters with the most speaking time**, and group all other characters into a single category.
- The coloring of the pie chart should be based on the `Gender` of the characters, but each character should have a different shade of the color.
- Use the `ggplot2` package for the visualization, and the `fct_lump()` function to group the characters with the least speaking time into the "other" category.

```{r top 9 by book chart}
...
- Calculate the percentage of Frodo's words in each chapter, compared to the total number of words spoken in that chapter.
