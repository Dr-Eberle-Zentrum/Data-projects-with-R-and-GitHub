---
title: "Movies and Inflation Solution"
...
knitr::opts_chunk$set(
  echo = FALSE,    # Versteckt den R-Code
  warning = FALSE, # Versteckt alle Warnmeldungen (z.B. gelöschte Zeilen im Plot)
  message = FALSE  # Versteckt Paket-Lademeldungen (z.B. von tidyverse)
)
```
...
```{r top 10}
# Extract the top 10 highest-scoring movies
top10_movies <- movies_analyzed %>%  
  arrange(desc(total_score)) %>%  
  slice(1:10) %>%  # Selects the top 10 rows
  select(title, release_year, movie_rating, metascore, total_score)
...
movies_clean %>% 
  ...
  ggplot(aes(x = avg_score, y = director)) +
    geom_col(fill = "steelblue", alpha = 0.9, width = 0.1) +
  # Add whiskers 
    geom_errorbar(
...
```{r remove films without revenue}
movies_clean %>% 
  mutate(
    gross_num = as.numeric(gross_equivalent),
    total_score = movie_rating + metascore
  ) %>% 
  filter(!is.na(gross_num), gross_num > 0) %>% 
  unnest(genre_list) %>% 
  group_by(gross_num, total_score) %>% 
  slice_head(n = 1) %>% 
  ungroup() %>% 

  ggplot(aes(x = gross_num, y = total_score, color = genre)) +
  geom_point(alpha = 0.5, size = 2) + 
  geom_smooth(method = "lm", formula = y ~ x, color = "black", se = TRUE) +
  scale_x_sqrt(labels = scales::label_dollar(scale = 1e-6, suffix = "M")) +
  scale_color_brewer(palette = "Dark2") + 
