...
all_roosters_clean2 <- all_rosters_clean%>%
   bind_rows(.id = "Team")%>%
...
*Note: Probably could done it already in the import part (bind all rosters into one table), but wasn't sure if i wont need it separated later*
...
now let's mutate some variable for the graph, lets's start with variable requested in `results`, ex. count how often each of them played at home/away and their wins/losses...
...
# could do an max and min for each set and do an max again on it but....
  ...
  summarise(avg_ballpoint_home = mean(as.numeric(Ballpunkte)))
  
  avg_ballpoint_away <- df %>%
  filter(Mannschaft2 == team) %>%
  mutate(Ballpunkte = str_extract(Ballpunkte, "(?<=:)(\\d+)")) %>%
  summarise(avg_ballpoint_away = mean(as.numeric(Ballpunkte)))
  
  avg_attendance_home <- df %>%
  filter(Mannschaft1 == team) %>%
  summarise(avg_attendance_home = mean(as.numeric(Zuschauerzahl)))
...
**Finally got the Result stuff ready, lets go for the next.**

****

Already got an idea, could go over the positions with a function.
Therefore lets get the unique psitions in a list first.

***

```{r all_positions, echo=TRUE}
...
Need to join topscorer and roster to get all the team names into the topscorer file, there is already a column called `points_per_set` so if i get it done i just need to `group_by(Position)` and `Summarize(mean(Positon))`with lapply? over all teams.
```{r try}


mutate_team_games_var2 <- function(team, df) {
  avg_point_per_position <- df %>%
    filter(Mannschaft1 == team | Mannschaft2 == team) %>%
    group_by(Position) %>%
    summarise(
      Avg_Ballpunkte_first_set = round(mean(ifelse(Mannschaft1 == team, Satz1_Ballpunkte1, Satz1_Ballpunkte2)), 2)
    )
  return(avg_point_per_position)
}

attacke_variable_all_team_positions <- mapply(mutate_team_games_var2, team = all_games_teams, df = rep(list(all_topscorer_clean), length(all_games_teams)), SIMPLIFY = FALSE)


attacke_variable_all_team_positions
```


***

Rest of attacking values i can get from `all_games_clean`
with a `ifelse` over the Position of column `Ballpunkte`