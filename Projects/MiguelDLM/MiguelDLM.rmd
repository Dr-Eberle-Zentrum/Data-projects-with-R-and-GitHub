...
data_lion <- suppressMessages(read_csv(paste0(getwd(), "/smooth_stress_tensor (lion).csv")))
data_hyaena <- suppressMessages(read_csv(paste0(getwd(), "/smooth_stress_tensor (hyaena).csv")))
data_wolf <- suppressMessages(read_csv(paste0(getwd(), "/smooth_stress_tensor (wolf).csv")))
data_bear <- suppressMessages(read_csv(paste0(getwd(), "/smooth_stress_tensor (bear).csv")))
```
...
```{r plot_results_species}
#read the image (the same image will be used for all species)
img <- readPNG("Z_axe.png")
g <- rasterGrob(img, interpolate=TRUE)
#plot the changes in the average stress in the Z axis accros the groups using geom_points and conecting them with geom_line
#each species will be represented by a different line
p <- ggplot(combined_data_Z, aes(x = Z_group, y = average_stress, group = Species, color = Species)) +
    geom_point() +
    geom_line() +
    labs(title = "Average stress in the Z axis", x = "Z axis", y = "Average stress") +
    theme_minimal()
