...
#The holy pipe. Begins with mergeing 2019 and 2020 import/export data
data <- merge(Data2019, Data2020,  by=c("partner_code", "hs_product_code")) %>%
  ...
  merge(., HS_list, by.x="hs_product_code",by.y="code") %>%
  ungroup() %>%
  ...
  #Calculate Surplus/Deficit values
  mutate(
    Import_Diff = (total_import_2020 
                   - total_import_2019),
    Export_Diff = (total_export_2020 
                   - total_export_2019)
  ) %>%
...
codes <- unique(data$individual)
table <- HS_list[HS_list$code %in% codes,] %>%
...
```{r, echo=FALSE}
ggsave("CSB.jpg", width = 30, height = 20, units = "cm")
```
