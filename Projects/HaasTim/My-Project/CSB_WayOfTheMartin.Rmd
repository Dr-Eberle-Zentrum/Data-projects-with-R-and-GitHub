...
data <- inner_join(Data2019, Data2020,  by=c("partner_code", "hs_product_code")) %>%
...
table <- HS_list[HS_list$code %in% unique(data$individual),] %>%
...
#The plot call below is copied almost exactly from RGG
...
p <- ggplot(data) +      
  ...
  geom_text(data=label_data, aes(x=id, y=tot+10, label=paste0("Code: ",individual), hjust=hjust),
            color="black", fontface="bold",alpha=0.6, size=4, 
            angle= label_data$angle, inherit.aes = FALSE ) +
  
  geom_text(data=label_data, aes(x=id, y=800000000, label=as.character(ifelse(tot!=0, format(round(tot/1000000000, 2), nsmall = 2), "")), hjust=0),
...
colour = "black", alpha=0.6, size=1 , inherit.aes = FALSE )  +
geom_text(data=base_data, aes(x = title, y = -5000000000, label=group),
colour = "black", alpha=0.6, size=2.5, fontface="bold", inherit.aes = FALSE)
```

```{r, echo=FALSE}
ggsave("CSB_WayOfTheMartin.jpg", width = 30, height = 20, units = "cm")
```
