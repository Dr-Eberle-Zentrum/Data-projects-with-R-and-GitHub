...
#   summarise(accuracy =mean(correct,na.rm=T),timezone=format(min(timestamp),'%H'),rt=rt)%>%
#   arrange(timezone)
# 
# back_3<- dataRaw %>% 
#   select(!time_elapsed & !trial_type) %>%
...
# p1<- 
  ggplot(back_1, aes(x=timezone, y=rt,fill=timezone)) +
  geom_boxplot()+
...
ggplot(back_1, aes(x=timezone, y=accuracy)) +
  geom_boxplot()+
