...
```{r Threshold, datalonger and nesting}
...
```{r Prepare data and countries, message=FALSE, warning=FALSE}
...
```{r Facet plotting of countries, fig.width=10, fig.height=6}
...
The predictive power of this polynomial model is heavily restricted which can be seen by the outcomes for the United States (heavily declining), Camboda (even negative), Russia (stagnating) and Canada (heavily influenced by an outlier).

## 6) Plot world map

```{r World map heat map, fig.width=10, fig.height=6}
...
Countries close to the equator seem to suffer from slightly stronger tree loss.