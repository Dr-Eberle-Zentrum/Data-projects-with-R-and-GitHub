...
data <- read_csv2(here("Projects", "shad-lad-flad", "wows_playerbase_data.CSV"), na = "-") %>% 
  ...
  # Für jeden Typ innerhalb eines Tiers die Nation mit den meisten Battles ermitteln
  group_by(Tier, Type) %>%
  ...
  # Berechne die Punkte für jede Nation und jedes Tier
  group_by(Nation, Tier) %>%
  ...
  # Berechne total_points_sum für jede Nation (einmal pro Nation, nicht mehrfach)
  group_by(Nation) %>%
...
data2 <- read_csv2(here("Projects", "shad-lad-flad", "wows_playerbase_data.CSV"), na = "-") %>% 
  select(Nation, Tier, Type, Battles, `Win rate`, `Avg. damage`) %>% 
  ...
  group_by(Nation, Tier) %>% 
  summarize(total_points_tier = sum(points, na.rm = TRUE), .groups = "drop") %>% 
  
  # Gesamte Punkte für jede Nation berechnen
  group_by(Nation) %>% 
  mutate(total_points_sum = sum(total_points_tier, na.rm = TRUE)) %>% 
  ungroup() %>% 
...
```{r visualisation2, echo = FALSE, warning = FALSE, message = FALSE}
ggplot(data2, aes(x = reorder(Nation, total_points_sum), y = total_points_tier, fill = factor(Tier))) +
  geom_bar(stat = "identity", position = "stack", color = "black") +  # Balken mit schwarzem Rand
  geom_text(aes(label = case_when(
    total_points_tier == 0 ~ "",  # Wenn 0 Punkte, keinen Text anzeigen
    total_points_tier == 1 ~ "1", 
    total_points_tier == 2 ~ "2", 
    total_points_tier == 3 ~ "3", 
    total_points_tier == 4 ~ "4", 
    total_points_tier == 5 ~ "5 points", 
    total_points_tier == 6 ~ "6 points", 
    total_points_tier == 7 ~ "7 points", 
    total_points_tier == 8 ~ "8 points", 
    total_points_tier == 9 ~ "9 points", 
    TRUE ~ as.character(total_points_tier)
  )), position = position_stack(vjust = 0.5), color = "black", size = 3) +  # Text in Balken
  # Total Points Sum hinter die Balken schreiben
  geom_text(aes(
    x = reorder(Nation, total_points_sum),
    y = total_points_sum + 0.5,  # Position oberhalb der Balken
    label = total_points_sum
  ), hjust = 0, color = "black", size = 4, fontface = "bold") + 
  labs(
    title = "Points per Nation",
    subtitle = "Optional Point Modifier Applied",
    x = "Nation",
    y = "Points",
    fill = "Tier"
  ) +
