...
for (id in list.files(".", pattern = "^subject_.*\\.csv$")){
  subjects[[id]] <- readr::read_csv(id, show_col_types = FALSE)
}

all_subjects <- bind_rows(subjects)

PANAS <- readr::read_csv("PANAS_scores.csv", show_col_types = FALSE)
BDI <- readr::read_csv("BDI_scores.csv", show_col_types = FALSE)
...
ggsave("plots/visualization1.png", width = 10, height = 10, dpi = 300)
