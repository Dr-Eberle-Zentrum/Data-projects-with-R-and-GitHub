...
# calculating the number of cases and deaths per country with group_by() and mutate()
  ...
  summarise(cases_total = sum(cases),
            ...
            cases_total_per_capita = cases / population_2020 * 100,
            deaths_total_per_capita = deaths / population_2020 * 1000,
            population_total = sum(population_2020)) %>%
  ungroup()
...
corona_bip_eu <- right_join(corona_eu,bip_eu, by = "country")

```

```{r visualisation_of_cases_vs_bip}

ggplot(corona_bip_eu, aes(x = bip, y = cases_total_per_capita)) +
  geom_point() +
