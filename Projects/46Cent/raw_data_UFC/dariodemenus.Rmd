...
ggplot(map_data_joined, aes(long, lat, group = group, fill = n)) +
  ...
  # ---------------------------------------------------------
  # CHANGE: Added trans = "log10" to handle the USA outlier
  # Added 'breaks' so the legend shows readable numbers (1, 10, 100...)
  # ---------------------------------------------------------
...
ggplot(us_map_joined, aes(long, lat, group = group, fill = n)) +
  ...
  # ---------------------------------------------------------
  # CHANGE: Added log10 scale for US States as well.
  # Adjusted breaks to fit the state-level distribution 
  # (e.g., 1, 10, 50, 200) to capture low, medium, and high frequency states.
  # ---------------------------------------------------------
  scale_fill_viridis_c(
    na.value = "grey90", 
    name = "Events", 
    trans = "log10", 
    breaks = c(1, 5, 20, 100, 300)
  ) +
  labs(title = "UFC Events by US State (Log Scale)") +
  theme_void() + 
  coord_fixed(1.3) # Optional: Makes the map projection look less "stretched"
```
```{r Visualization_4}
fights_clean %>%
  count(Method_Simple) %>%
