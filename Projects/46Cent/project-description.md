...
Other than the cleaning we have to do, the first step will be to add
additional variables of interest,  
for example `height_reach_ratio` or `homecountry_gdp`.  
Based on these we could then create plots, which aren’t already
accessible online.  
Creating a predictive model might be a too ambitious goal (yet), however
we definitely put the fundamental requirements for this into place.  

## Visualization Goals

Not only would it be interesting to visually experience how far the top
fighters are from the average (e.g. in terms of W/L-Ratio)  
but also more demographic insights, like the potential negative
correlation between a fighter’s home country GDP and his W/L-Ratio.  
(E.g a Line-/Scatterplot were some outliers are specifically mentioned
by name).
